...
/**
 * Class 4 page with dynamic content based on the current date
 */
export default function Class4Page() {
  const router = useRouter();
  const classIndex = 3; // This is class-4
  ...
  useEffect(() => {
    ...
    console.log('===== DEBUG INFO (Class 4) =====');
        ...
        <div className="text-center mb-8">
          ...
          {classStatus === 'live-today' && (
            <div className="bg-green-600 text-white py-3 px-4 rounded-lg mb-4 animate-pulse">
              <p className="text-xl font-bold">Class 4 is live today!</p>
            </div>
          )}
          {classStatus === 'live-tomorrow' && (
            <div className="bg-yellow-600 text-white py-3 px-4 rounded-lg mb-4">
              <p className="text-xl font-bold">Class 4 is live tomorrow!</p>
            </div>
          )}
          {classStatus === 'live-on-date' && (
            <div className="bg-blue-600 text-white py-3 px-4 rounded-lg mb-4">
              <p className="text-xl font-bold">Class 4 is live on {COURSES[classIndex].date}!</p>
            </div>
          )}
